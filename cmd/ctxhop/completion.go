...
func renderBashCompletion() string {
	...
	out.WriteString(`# bash completion for ctxhop
_ctxhop_completion() {
...
complete -F _ctxhop_completion ctxhop
`)
	return out.String()
}
...
func renderZshCompletion() string {
	var out strings.Builder
	out.WriteString(`#compdef ctxhop
...
`)
    ...
    _describe 'ctxhop values' candidates
}

compdef _ctxhop ctxhop
`)
	return out.String()
}
...
func renderFishCompletion() string {
	var out strings.Builder
	out.WriteString(`# fish completion for ctxhop
complete -c ctxhop -f -n '__fish_use_subcommand' -a '`)
...
`)
	for _, command := range completionCommandNames() {
		...
		fmt.Fprintf(&out, "complete -c ctxhop -f -n '__fish_seen_subcommand_from %s' -a '%s'\n", command, completionWords(candidates))
	}
	return out.String()
}
...
func renderPowerShellCompletion() string {
	var out strings.Builder
	out.WriteString(`# PowerShell completion for ctxhop
Register-ArgumentCompleter -Native -CommandName ctxhop -ScriptBlock {
