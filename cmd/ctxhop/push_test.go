...
func TestParsePushOptionsUsesWorkspaceFlag(t *testing.T) {
	options, err := parsePushOptions([]string{"--workspace"})
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, removed := range []string{"--include-project-state", "--include-workspace", "--include-git-state"} {
		if _, err := parsePushOptions([]string{removed}); err == nil {
			t.Fatalf("push accepted the removed %s flag", removed)
		}
	}
	options, err = parsePushOptions([]string{"--workspace", "native-session"})
	if err != nil || options.session != "native-session" || !options.workspace {
		t.Fatalf("positional session options = %+v, error = %v", options, err)
	}
	options, err = parsePushOptions([]string{"--git-stash", "stash@{0}"})
	if err != nil || !options.workspace {
		t.Fatalf("Git stash did not enable workspace upload: %+v, error = %v", options, err)
	}
	if _, err := parsePushOptions([]string{"--session", "native-session"}); err == nil {
		t.Fatal("push accepted the removed --session flag")
	}
}
