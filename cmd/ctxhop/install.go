...
func runInstall(args []string) error {
	...
	if _, err := fmt.Fprintf(os.Stdout, "%s: installed command at %s\n", cliName, result.targetPath); err != nil {
		return err
	}
	if options.noPath {
		_, err := fmt.Fprintf(os.Stdout, "%s: PATH was not changed because --no-path was supplied\n", cliName)
		return err
	}
...
func installExecutableFile(sourcePath, targetPath string) error {
	...
	temporary, err := os.CreateTemp(filepath.Dir(targetPath), ".ctxhop-install-*")
	if err != nil {
		return err
	}
	...
	if _, err := os.Lstat(targetPath); err == nil {
		backup, createErr := os.CreateTemp(filepath.Dir(targetPath), ".ctxhop-install-backup-*")
		if createErr != nil {
			return createErr
		}
