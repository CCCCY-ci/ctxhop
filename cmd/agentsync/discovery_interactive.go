...
func discoveryCandidates(path []string, query string) []discoveryCandidate {
	...
	if len(path) == 0 {
		...
		for _, action := range completionSubcommands[path[0]] {
			name := path[0] + " " + action
			candidates = append(candidates, discoveryCandidate{name: action, summary: discoverySummary(name)})
		}
	}
...
func renderCommandPicker(output io.Writer, path []string, query string, candidates []discoveryCandidate, selected int) error {
	...
	if len(path) == 0 {
		if _, err := fmt.Fprintln(output, "AgentSync commands"); err != nil {
			return err
		}
	} else {
		if _, err := fmt.Fprintf(output, "AgentSync %s commands\n", strings.Join(path, " ")); err != nil {
			return err
		}
	}
