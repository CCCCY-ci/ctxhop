...
func TestWriteWatchEventTextAndJSON(t *testing.T) {
	...
	for _, event := range []watchEvent{
		...
		{Scope: "project", Event: "error", Error: "push cycle failed; run 'agentsync push' for details"},
	} {
...
func TestSafeWatchErrorDoesNotExposeDetails(t *testing.T) {
	cases := []struct {
		...
		{err: errors.New("backend password=secret path=C:\\private"), want: "push cycle failed; run 'agentsync push' for details"},
	}
