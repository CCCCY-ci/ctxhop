...
func TestDeviceRemoveRotatesKeyAndRevokesDevice(t *testing.T) {
	...
	if err := runInitWithIO([]string{"--backend", "dir", "--path", remoteRoot, "--device-name", "first", "--no-hook"}, firstInput, ioDiscard{}, "test-agentsync"); err != nil {
	...
	if err := runInitWithIO([]string{"--invite", invitePath, "--device-name", "second", "--no-hook"}, secondInput, ioDiscard{}, "test-agentsync"); err != nil {
	...
	t.Setenv("AGENTSYNC_CONFIG_DIR", thirdConfig)
	thirdInput := strings.NewReader(newPass + line + newPass + line)
	if err := runInitWithIO([]string{"--invite", invitePath, "--device-name", "third", "--no-hook"}, thirdInput, ioDiscard{}, "test-agentsync"); err != nil {
