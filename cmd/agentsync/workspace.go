...
func init() {
	for i := range commands {
		if commands[i].name == "workspace" {
			commands[i].run = runWorkspace
		}
	}
}
...
func runWorkspaceWithStreams(args []string, input io.Reader, output, prompt io.Writer) error {
	...
	if options.action == "apply" {
		if !options.yes {
			...
			report.Notes = append(report.Notes, "no files changed; rerun with 'workspace apply --yes' to write available file bodies")
		} else {
...
func parseWorkspaceOptions(args []string) (workspaceOptions, error) {
	...
	yes := flags.Bool("yes", false, "confirm writing available workspace files")
	if err := flags.Parse(args[1:]); err != nil {
		return workspaceOptions{}, fmt.Errorf("workspace %s: %w", action, err)
	}
	if action == "preview" && *yes {
		return workspaceOptions{}, errors.New("workspace preview: --yes is only valid with apply")
	}
	if flags.NArg() != 1 {
	...
	return workspaceOptions{action: action, session: session, json: *jsonOutput, yes: *yes}, nil
}
...
func readWorkspaceSnapshotForSession(ctx context.Context, state environmentContext, session *listSession) (workspacepkg.Snapshot, error) {
	...
	return workspacepkg.Snapshot{}, errors.New("workspace: no explicit workspace snapshot is available; run 'agentsync push --include-workspace' on the source device")
}
...
func applyWorkspaceSnapshot(state environmentContext, snapshot workspacepkg.Snapshot, report *workspacePreviewReport) error {
	if report == nil {
		return errors.New("workspace apply: report is required")
	}
