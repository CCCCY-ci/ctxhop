...
func TestDetect(t *testing.T) {
	...
	l := Layout{Home: home, version: fixedVersion("2.1.226")}
...
func TestGradeVersion(t *testing.T) {
	...
	for _, tt := range tests {
		got, reason := gradeVersion(tt.version)
		if got != tt.want {
			t.Errorf("gradeVersion(%q) = %v, want %v", tt.version, got, tt.want)
		}
		if reason == "" {
			t.Errorf("gradeVersion(%q) gave no reason", tt.version)
		}
		// Reasons reach `agentsync doctor`, whose output must be safe to paste
		// into a public issue.
		if strings.ContainsAny(reason, `\/`) {
			t.Errorf("reason looks like it contains a path: %q", reason)
		}
	}
}

func TestUnknownVersionDoesNotStopBackup(t *testing.T) {
	// Agents ship constantly. Refusing everything on each release would strand
	// users far more often than it would protect them, so an unknown version
	// restricts writing, not reading.
	got, _ := gradeVersion("99.0.0")
	if got == CompatStopped {
		t.Error("an unknown version must not stop the adapter outright")
	}
}

func TestGradeSession(t *testing.T) {
	t.Run("no findings leaves the level alone", func(t *testing.T) {
		got, reason := GradeSession(CompatFull, nil)
		if got != CompatFull || reason != "" {
...
func TestVersionIsEmptyWithNothingRecorded(t *testing.T) {
	...
	// Least information must not mean least restriction.
	if inst.Compatibility != CompatLimited {
		t.Errorf("Compatibility = %v, want CompatLimited", inst.Compatibility)
	}
}
