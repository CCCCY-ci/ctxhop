#!/usr/bin/env bash
#
# Cross-compile agentsync for every supported target from a single machine.
#
# CGO is disabled on purpose. It is what makes cross-compilation a one-liner and
# what guarantees a single statically linked binary per platform, which is the
# whole distribution story for a CLI tool. Any dependency that requires cgo
...
PKG="github.com/CCCCY-ci/agentsync/cmd/agentsync"
...
for target in "${TARGETS[@]}"; do
  ...
  out="dist/agentsync_${GOOS}_${GOARCH}"
